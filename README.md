...
## 🚀 O papel deste serviço (`auth-service`)
O **Auth Service** é o pilar de segurança da solução. Suas responsabilidades incluem:
*   Gerenciar o cadastro e login de usuários.
*   Emitir e validar tokens **JWT (JSON Web Token)**.
*   Garantir que todas as requisições para a API de vídeos sejam devidamente autorizadas.
*   Utilizar o **Redis** para gerenciamento de sessões ou blacklist de tokens, se necessário.
...
*   **AWS API Gateway:** Ponto de entrada único para o ecossistema. Realiza o roteamento do tráfego externo para dentro da VPC através de **VPC Links**, garantindo que os microsserviços não fiquem expostos diretamente à internet pública.

### Orquestração e Compute
*   **Amazon EKS (Elastic Kubernetes Service):** Cluster gerenciado que orquestra a execução dos microsserviços. Permite o **Horizontal Pod Autoscaling (HPA)**, escalando os workers de processamento conforme a demanda de mensagens na fila.

### Microsserviços do Ecossistema
*   **`fiapx-app-auth-service`** (este serviço): Responsável pelo cadastro e autenticação de usuários via JWT.
*   **`fiapx-app-video-api`**: Interface de upload e consulta de status.
*   **`fiapx-app-worker-processor`**: Serviço de background que consome as mensagens e processa o vídeo.

### Persistência e Mensageria
*   **RabbitMQ (Amazon MQ):** Broker de mensagens que garante a resiliência do sistema.
...
1.  **🔒 Autenticação:** O usuário obtém um token JWT através deste `auth-service`.
2.  **📤 Upload:** O usuário envia o vídeo para a `video-api` portando o JWT válido.
3.  **📨 Enfileiramento:** A API publica uma mensagem no RabbitMQ.
4.  **⚙️ Processamento:** O `worker-processor` executa a extração das imagens.
5.  **✅ Finalização:** O worker atualiza o status final.
